...
// 	uint8_t data[100];
// } data_and_length;
// data_and_length in_data;
//
// static uint8_t incomingSize;
// static uint8_t incomingData[16];
...
// 	while (1) {
// 		if (incomingSize > 0) {
// 			int length = read(ctx->fd, incomingData, incomingSize);
// 			if (length <= 0) {
// 				csp_log_error("%s: read() failed, returned: %d", __FUNCTION__, length);
// 				incomingSize = 0;
// 				exit(1);
// 			}
// 			// ctx->rx_callback(ctx->user_data, incomingData, incomingSize, NULL);
// 			// csp_usart_write(ctx->fd, incomingData, incomingSize)
// 			printf("%d\n", incomingData[0]);
// 			incomingSize = 0;
// 		} else {
// 			int length = read(ctx->fd, &incomingSize, sizeof(uint8_t));
// 			if (length <= 0) {
// 				csp_log_error("%s: read() failed, returned: %d", __FUNCTION__, length);
// 				exit(1);
// 			}
...
// 	uint8_t data[16] = (uint8_t *)dat
// 	if (fd >= 0) {
// 		printf("%d\n", (uint8_t) data);
// 		int res = write(fd, (void*)&size, sizeof(uint8_t));
