...
static PyObject* pycsp_can_socketcan_init(PyObject *self, PyObject *args) {
    ...
    int can_id = 0;
    int mask = 0;
    if (!PyArg_ParseTuple(args, "s|iiii", &ifc, &bitrate, &promisc, &can_id, &mask)) {
        return NULL;
    }

    int res = csp_can_socketcan_open_and_add_interface(ifc, CSP_IF_CAN_DEFAULT_NAME, bitrate, promisc, NULL, can_id, mask);
    if (res != CSP_ERR_NONE) {
